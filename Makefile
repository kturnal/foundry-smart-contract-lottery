...
# Phony targets dont require paths, instead commands are searched inside the Makefile and run as the command specified from here.
...
# conditional to check if the network is sepolia
ifeq ($(findstring --network sepolia,$(ARGS)),--network sepolia) 
	NETWORK_ARGS := --rpc-url $(SEPOLIA_RPC_URL) --private-key $(PRIVATE_KEY) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvv
endif

# @ sign obfuscates the command from the output
deploy:
